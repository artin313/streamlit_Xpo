...
st.title('Real Estate Price Pedictor')
...
def get_user_input():
    ...
    avg_room = st.sidebar.number_input('Avg. Number of Rooms')
    avg_bedrooms = st.sidebar.number_input('Avg. Number of Bedrooms')
    area_population = st.sidebar.number_input('Area Population')
    return np.array([avg_income,avg_age,avg_room,avg_bedrooms,area_population]).reshape(1,-1)
